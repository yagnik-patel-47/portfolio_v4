		...
		>
			...
			>
				<AnimatePresence mode="popLayout" initial={false}>
					...
					{state && (
						<m.div layout layoutId="container" key={`cont`}>
							<m.div
								layout
							...
							<m.div layout className="grid grid-cols-4 gap-4 p-6 relative">
								{peps.map((pep) => (
									<m.div
										layout
										key={pep.name + "con"}
							...
							<m.div layout className="space-y-3 pb-6 px-6 relative">
								<m.button
									layout
									onClick={() => {
								...
								<p className="text-center text-neutral-600 font-medium text-sm">
									{newJoined ? "Mic is muted." : "Mic will be muted initially."}
								</p>
							</m.div>
						</m.div>
					)}
